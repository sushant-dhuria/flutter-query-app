...
class _DSAState extends State<DSA> {
  ...
  Future<List<Singlequery>> fetchjson() async {
    ...
    var res = await http.get("http://localhost:8080/queriesDSA");
    // print(res.body);
    List<Singlequery> queriesDSA = [];
    _queriesDSA = [];
    if (res.statusCode == 200) {
      var queriesjson = json.decode(res.body);
      for (var Singlequeryjson in queriesjson) {
        queriesDSA.add(Singlequery.fromJson(Singlequeryjson));
      }
    }
    return queriesDSA;
  }
  ...
  void initState() {
    fetchjson().then((value) {
      setState(() {
        _queriesDSA.addAll(value);
        temp.addAll(_queriesDSA);
      });
    });
    super.initState();
  }
                ...
                itemBuilder: (_, index) {
                  ...
                  return Container(
                    ...
                    child: Container(
                      ...
                      child: ListTile(
                        onTap: () => {
                          qu = _queriesDSA[index].id.toString(),
                          Navigator.push(
                              context,
                              MaterialPageRoute(
                        ...
                        title: Text(_queriesDSA[index].q.toString()),
